from ziprecruiter.ziprecruiter.ziprecruiter import ZipRecruiter
from monster.monster.monster import Monster
from tqdm import trange, tqdm
import schedule, time, traceback
import threading

SITES = {"ziprecruiter": ZipRecruiter, "monster": Monster}
...
class Japplr:
    ...
    def api_throttle_secs(self, val):
        ...
        if not self.__logged_in:
            ...
            raise Exception(
                "ERROR : API THROTTLE SECS CANNOT BE EDITTED ONCE LOGGED IN"
            )
        ...
        for site, account in self.accounts.items():
            if site in SITES:
                ...
                if "enabled" in account and not account["enabled"]:
                    print(
                        "INFO  : {0} : ACCOUNT DISABLED, SKIPPING LOGIN".format(
                            site.title()
                        )
                    )
                    continue
                ...
                if not site_instance.login(account["email"], account["password"]):
                    raise ValueError(
                        "ERROR : {0} : COULD NOT LOGIN TO YOUR ACCOUNT : {1} : {2}".format(
                            site, str(account["email"]), account["password"]
                        )
                    )
                else:
                ...
                print(
                    "INFO  : {0} : SITE NOT SUPPORTED. SKIPPING LOGIN".format(
                        site.title()
                    )
                )
    ...
    def run(
        self, quantity_per_search=10, schedule_every_mins=30, give_up_on_search_secs=0
    ):
        ...
        def apply_to_jobs_for_site(site):
            ...
            pbar = trange(
                total, desc=site.title() + " | Applying", unit="Jobs", leave=False
            )

            for search in self.searches:
                pbar.set_description(
                    site.title() + " | " + search["keywords"].title() + " | Applying"
                )
                pbar.refresh()
                ...
                def run_search_and_apply():
                    ...
                    try:
                        search_results = self.sites[site].search(
                            quantity_per_search, **search_filters
                        )
        ...
        for site in self.sites:
            t = threading.Thread(
                target=apply_to_jobs_for_site, args=(site,)
            ...
            if schedule_every_mins > 0:
                schedule.every(schedule_every_mins).minutes.do(
                    apply_to_jobs_for_site, site=site
